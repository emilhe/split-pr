...
`--remainder` for unassigned hunks, `--dep "from:to"` for edges.
...
`is_shared`, and `key_files` per topic (use the Write tool). Then:

```bash
split-pr-tools update-metadata $RUN/discovery.json $RUN/metadata.json
```

Every topic must have a meaningful description — a paragraph for reviewers
